                ...
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {currentRepos.map(repo => (
                            <tr onClick={() => { navigate(`/repo/${repo.id}`) }} className="border-b dark:bg-gray-800 dark:border-gray-700 odd:bg-white even:bg-gray-50 odd:dark:bg-gray-800 even:dark:bg-gray-700 hover:bg-orange-300 hover:cursor-pointer">
                                <th scope="row" className="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                    {repo.name}
                                </th>
                                <td className="px-6 py-4">
                                    {repo.description || <i className="font-light">no description found</i>}
                                </td>
                                <td className="px-6 py-4">
                                    {repo.language}
                                </td>
                                ...
                                <td className="px-6 py-4 text-right">
                                    <Link to={`/repo/${repo.id}`}><a className="font-medium text-blue-600 dark:text-blue-500 hover:underline">View Repo</a></Link>
                                </td>
                            </tr>
                        ))}
